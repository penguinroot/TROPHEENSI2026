...
class AffichageTeteHaute:
    ...
    def mettre_a_jour(self):
        self.texte.text = (
            ...
            f"Carte SD {self.camera_equipement.photos_prises}/{self.camera_equipement.capacite}"
        )
...
def creer_menu_dialogue(gest_menus, gest_notifs, etat_jeu, appareil_photo):
    panneau = Entity(
        ...
        scale=(0.70, 0.55), position=(0, 0), z=0.4, enabled=False
    )
    
    Text("Garde Forestier", parent=panneau, y=0.40, origin=(0, 0),
         scale=2.0, color=Couleurs.TITRE, z=-0.1)
    
    texte_dialogue = Text(
        "", parent=panneau,
        y=0.15, origin=(0, 0),
        scale=1.2, color=Couleurs.TEXTE, z=-0.1
    )
    ...
    DIALOGUES = {
        ...
        "appat": {
            "texte": "Tiens prends ca\n\nUtilise-le bien les animaux meritent le respect",
            "effet": donner_appat,
            "suite": "fin"
        },
        "info": {
    ...
    def afficher_etape(etape_id):
        ...
        if "choix" in data:
            ...
            for i, (label, cle) in enumerate(data["choix"]):
                y_pos = debut_y - i * 0.12
                b = creer_bouton(
                    label, panneau,
                    pos=(0, y_pos),
                    taille=(0.58, 0.10),
                    au_clic=lambda c=cle: afficher_etape(c)
                )
...
def creer_menu_commandes(gest_menus):
    panneau = Entity(parent=camera.ui, model='quad', color=Couleurs.PANNEAU,
                     scale=(0.62, 0.85), position=(0, 0), z=0.4, enabled=False)
...
def creer_menu_encyclopedie(gest_menus, etat_jeu, entites):
    panneau = Entity(parent=camera.ui, model='quad', color=Couleurs.PANNEAU,
                     scale=(0.90, 0.88), position=(0, 0), z=0.4, enabled=False)
...
def creer_menu_boutique(gest_menus, etat_jeu, appareil_photo, gest_notifs):
    panneau = Entity(parent=camera.ui, model='quad', color=Couleurs.PANNEAU,
                     scale=(0.55, 0.52), position=(0, 0), z=0.4, enabled=False)
...
def creer_menu_quiz(gest_menus, gest_notifs, etat_jeu):
    panneau = Entity(parent=camera.ui, model='quad', color=Couleurs.PANNEAU,
                     scale=(0.52, 0.55), position=(0, 0), z=0.4, enabled=False)
    Text("Quiz Nature", parent=panneau, y=0.40, origin=(0, 0),
         scale=2.5, color=Couleurs.TITRE, z=-0.1)
    ...
    def repondre(rep):
        if rep == panneau.reponse_attendue:
            ...
            gest_notifs.ajouter(
                f"Erreur C'etait un {panneau.reponse_attendue}",
                Couleurs.ATTENTION
            )
...
class JeuFaunex:
    ...
    def _creer_entites_monde(self):
        donnees_animaux = [
            ...
            ("Ours Brun", "Mammifere", color.brown, (0, 1.5, 0), "fuit", 4),
            ("Cerf", "Mammifere", color.rgb(160, 100, 40), (0, 1.5, 0), "fuit", 2),
            ("Fennec", "Mammifere", color.yellow, (0, 1.0, 0), "fuit", 3),
            ("Loup Gris", "Mammifere", color.gray, (0, 1.0, 0), "fuit", 4),
            ("Sanglier", "Mammifere", color.rgb(100, 70, 50), (0, 1.0, 0), "fuit", 2),
            ("Lynx", "Mammifere", color.black, (0, 1.0, 0), "fuit", 5),
            ("Panthere", "Mammifere", color.white, (0, 1.0, 0), "fuit", 5),
            ("Aigle Royal", "Oiseau", color.rgb(200, 150, 50), (0, 15.0, 0), "fuit", 4),
            ("Faucon", "Oiseau", color.dark_gray, (0, 20.0, 0), "fuit", 5),
            ("Corbeau", "Oiseau", color.black, (0, 12.0, 0), "fuit", 1),
            ("Hibou", "Oiseau", color.rgb(130, 100, 60), (0, 10.0, 0), "sommeil", 3),
            ("Pigeon", "Oiseau", color.light_gray, (0, 8.0, 0), "fuit", 1),
            ("Crocodile", "Reptile", color.rgb(50, 100, 50), (0, 0.5, 0), "sommeil", 4),
            ("Vipère", "Reptile", color.rgb(120, 150, 80), (0, 0.2, 0), "fuit", 3),
            ("Caméléon", "Reptile", color.green, (0, 0.5, 0), "sommeil", 4),
            ("Iguane", "Reptile", color.rgb(80, 180, 80), (0, 0.5, 0), "fuit", 3),
            ("Scorpion", "Insecte", color.red, (0, 0.2, 0), "fuit", 3),
            ("Scarabée", "Insecte", color.rgb(20, 20, 80), (0, 0.1, 0), "fuit", 1),
            ("Mante", "Insecte", color.lime, (0, 0.5, 0), "sommeil", 3),
            ("Papillon", "Insecte", color.cyan, (0, 3.0, 0), "fuit", 2),
        ]
        ...
        for d in donnees_animaux:
            x, z = self._generer_position_aleatoire(rng, positions_occupees)
            d = list(d)
            d[3] = (x, d[3][1], z)
        ...
        for _ in range(3):
            x, z = self._generer_position_aleatoire(rng, positions_occupees, dist_min=10)
            self.entites.append(Dechet((x, 0.5, z)))
            positions_occupees.append((x, 0, z))
    ...
    def _generer_position_aleatoire(self, rng, positions_existantes, dist_min=8, rayon=ParametresJeu.RAYON_MONDE):
        for _ in range(100):
            x = rng.uniform(-rayon, rayon)
            z = rng.uniform(-rayon, rayon)
            if (x ** 2 + z ** 2) ** 0.5 < ParametresJeu.DIST_MIN_SPAWN:
                continue
            trop_proche = False
            for px, _, pz in positions_existantes:
                if ((x - px) ** 2 + (z - pz) ** 2) ** 0.5 < dist_min:
                    trop_proche = True
                    break
            if not trop_proche:
                return x, z
        return rng.uniform(-rayon, rayon), rng.uniform(-rayon, rayon)
    ...
    def update(self):
        ...
        if time.time() - self.temps_derniere_sauvegarde >= 10:
            ...
            self.temps_derniere_sauvegarde = time.time()
            self.gest_notifs.ajouter("Sauvegarde automatique...", color.gray, 1.5)
    ...
    def input(self, key):
        ...
        if key == 'scroll up':
            ...
            if self.etat_jeu.appats_restants > 0:
                ...
                self.appats.append(nouvel_appat)
                self.gest_notifs.ajouter(
                    f"Appat pose ({self.etat_jeu.appats_restants} restant)"
                )
            else:
