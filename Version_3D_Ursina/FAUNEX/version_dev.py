...
def chemin_modele_ursina(chemin: Path):
    """Convertit un chemin OS en chemin relatif compatible Ursina."""
    try:
...
def charger_modele(nom, base_dir, mapping):
    """
    Charge un modèle en priorité OBJ+MTL, sinon GLB.
    Retourne le chemin relatif Ursina ou None.
    """
    ...
    try:
        ...
        if chemin_complet.suffix.lower() == '.obj':
            ...
            if mtl.exists():
                print(f"✓ OBJ+MTL : {chemin_relatif}")
            else:
                print(f"⚠️  MTL manquant : {chemin_complet}")
        else:
    ...
    def mettre_a_jour_ia(self, joueur, entites):
        ...
        if appats:
            ...
            if dist_vers_appat < ParametresJeu.DIST_ATTRACTION_APPAT:
                direction = Vec3(
                    le_plus_proche.x - self.x, 0, le_plus_proche.z - self.z
                ).normalized()
...
class AppareilPhoto:
    ...
    def prendre_photo(self, cible, etat_jeu):
        """Retourne (succes, est_nouveau, score)."""
...
class AffichageTeteHaute:
    def __init__(self, etat_jeu, appareil_photo):
        ...
        self.texte = Text(
            parent=camera.ui, position=(-0.85, 0.46), scale=1.2,
            color=Couleurs.TEXTE, z=-0.6
        )
...
def creer_menu_commandes(gest_menus):
    ...
    creer_bouton("Fermer (TAB)", panneau, (0, -0.40), (0.45, 0.085),
...
def creer_menu_boutique(gest_menus, etat_jeu, appareil_photo, gest_notifs):
    ...
    def acheter(objet, cout, action):
        if etat_jeu.credits >= cout:
            ...
            gest_notifs.ajouter(
                f"Pas assez de crédits ! (manque {cout - etat_jeu.credits} cr)",
                Couleurs.ATTENTION
            )
...
def creer_menu_quiz(gest_menus, gest_notifs, etat_jeu):
    ...
    def repondre(rep):
        if rep == panneau.reponse_attendue:
            etat_jeu.credits += 15
            gest_notifs.ajouter("Bonne réponse ! +15 cr", Couleurs.ACCENT)
        else:
            gest_notifs.ajouter(f"Erreur ! C'était un(e) {panneau.reponse_attendue}.", Couleurs.ATTENTION)
        gest_menus.fermer('quiz')

    for i, rep in enumerate(["Mammifère", "Reptile", "Insecte", "Oiseau"]):
...
class JeuFaunex:
    ...
    def _creer_entites_monde(self):
        donnees_animaux = [
            # Mammifères
            ("Renard Roux", "Mammifère", color.orange,             ( 0, 1.0,  0), "fuit",    2),
            ("Ours Brun",   "Mammifère", color.brown,              (10, 1.5,  0), "curieux", 4),
            ("Cerf",        "Mammifère", color.rgb(160, 100, 40),  (20, 1.5,  0), "fuit",    2),
            ("Fennec",      "Mammifère", color.yellow,             (30, 1.0,  0), "curieux", 3),
            ("Loup Gris",   "Mammifère", color.gray,               (40, 1.0,  0), "curieux", 4),
            ("Sanglier",    "Mammifère", color.rgb(100,  70, 50),  (50, 1.0,  0), "fuit",    2),
            ("Lynx",        "Mammifère", color.black,              (60, 1.0,  0), "curieux", 5),
            ("Panthère",    "Mammifère", color.white,              (70, 1.0,  0), "fuit",    5),
            # Oiseaux
            ("Aigle Royal", "Oiseau",    color.rgb(200, 150, 50),  ( 0,15.0,  0), "fuit",    4),
            ("Faucon",      "Oiseau",    color.dark_gray,          (10,20.0,  0), "fuit",    5),
            ("Corbeau",     "Oiseau",    color.black,              (20,12.0,  0), "curieux", 1),
            ("Hibou",       "Oiseau",    color.rgb(130, 100, 60),  (30,10.0,  0), "sommeil", 3),
            ("Pigeon",      "Oiseau",    color.light_gray,         (40, 8.0,  0), "curieux", 1),
            # Reptiles
            ...
            ("Papillon",    "Insecte",   color.cyan,               (30, 3.0, 20), "curieux", 2),
        ]
        ...
        for pos in [(-10, 0, 10), (20, 0, 30), (40, 0, -20), (-30, 0, -40), (60, 0, 0)]:
    ...
    def verifier_salutation_pnj(self):
        if self.etat_jeu.pnj_rencontre:
            return
        if distance_2d(self.joueur.position, self.pnj.position) < ParametresJeu.DIST_SALUTATION_PNJ:
    ...
    def update(self):
        ...
        # Autosauvegarde toutes les 10 secondes
        ...
        if key == 'scroll up':
            ...
            if self.etat_jeu.appats_restants > 0:
                ...
                self.gest_notifs.ajouter(
                    f"Appât posé ! ({self.etat_jeu.appats_restants} restant(s))"
                )
            else:
            ...
            if self.appareil_photo.en_mise_au_point:
                ...
                if (touche_ray.hit
                    ...
                    if not succes:
                        ...
                        self.gest_notifs.ajouter(
                            f"{touche_ray.entity.nom} déjà vu ! +{score} cr", Couleurs.TEXTE
                        )
